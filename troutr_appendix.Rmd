...
Combining all of this into Bayes Law gives normalized probability values for each candidate pair, plus an additional option that none are correct.  For example, a sample of the results is listed below (note that the probability sums to 1.0 for each trout water; in some cases this appears not to be the case due to rounding carried out for display purposes), 
...
p <- df.tst %>%
  ggplot() +
  geom_jitter(aes(x=jc, y=P), alpha=0.7) +
  theme_minimal()
```
