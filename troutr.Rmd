...
Geocoding the trout stocking locations was more challenging.  The DWR website offers an [interactive map](https://dgif-virginia.maps.arcgis.com/apps/webappviewer/index.html?id=441ed456c8664166bb735b1db6024e48) that allows users to search and view the appropriate water bodies, but there doesn't appear to be a straightforward way to export these for offline use.  Instead, the water bodies identified from the stocking schedule were manually searched on the map and the coordinates were recorded manually in an external file.  See Appendix for more details on the geocoding process.
...
df.cgy <- df.trout %>%
  ...
  group_by(cgy,wk,yr) %>%
  summarise(tot=n()) %>%
  pivot_wider(id_cols=c("wk","yr"), names_from=c("cgy"), values_from=c("tot"), values_fill=0) %>%
  arrange(wk) %>%
  full_join(df.tmp) %>%
  dplyr::select(-nn)
...
p.tst <- df.cgy %>%
  pivot_longer(cols=c("CR":"A")) %>%
  mutate(cgy = factor(name, levels=rev(lev))) %>%
  ggplot() +
  geom_area(aes(x=wk, y=value, ymin=0, group=name), fill="firebrick", color="grey25", alpha=0.7) +
  facet_grid(cgy~yr) +
  theme_minimal()
...
<ul>
  ...
  <li>[info sheet](https://dwr.virginia.gov/wp-content/uploads/media/catchable-trout-stocking-plan.pdf)</li>
<ul>
