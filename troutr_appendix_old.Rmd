...
df.deq2 <- df.deq %>%
  filter(WATER_NAME != "Unnamed tributary") %>%
...
names(vec_tokens) <- c(df.deq2$WQS_ID, df.trt2$id)
...
The result is a list of candidate pairs, nominally, pairs which will yield a jaccard similarity less than the threshold value implied by the choice of $b$.  With a little bit of post-processing one can generate a result analogous to what was produced above,
...
df.cnd2 <- candidates_040 %>%
  ...
  rename(id=a, WQS_ID=b) %>%
  left_join(df.trt %>% dplyr::select(id, waterbody1, county), by=c("id")) %>%
  left_join(df.deq %>% dplyr::select(WQS_ID, WATER_NAME, src, coord, county2=county), by=c("WQS_ID")) %>%
